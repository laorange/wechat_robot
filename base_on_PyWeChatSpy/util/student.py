...
class Student(StudentNoWechat):
    ...
    def send_schedule_auto(self, if_tomorrow: bool, date: str, week: int, what_day: str):
        ...
        if self.what_day == 'Sunday':
            if_send = False
            print('周日不发送')
        elif date in ['2020-10-01', '2020-10-01', '2020-10-01']:
            if_send = False
            print('国庆假期')

        if self.grade in ['2018', '2019', '2020'] and if_send:
            try:
                if len(self.c0.final_class_ch_name) + len(self.c1.final_class_ch_name) + \
                    ...
                    print('今天全天没有课')
                    send_msg_when(self.name, '今天全天没有课', date + ' 0' + self.send_hour + ':' + self.send_min + ':' + self.send_sec)
